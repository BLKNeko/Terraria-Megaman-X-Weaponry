...
namespace MegamanXWeaponry.Projectiles
{
	...
	public class FlashLaserCharged : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
            ...
            if (IsAtMaxCharge)
            {
                DrawLaser(spriteBatch, ModContent.Request<Texture2D>("FlashLaserCharged").Value, Main.player[Projectile.owner].Center,
		...
		public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50) {
			...
			for (float i = transDist; i <= Distance; i += step) {
                ...
                Main.EntitySpriteDraw(texture, origin - Main.screenPosition - F,
			...
			//spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition,
				spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition - F,
				new Rectangle(0, 0, 28, 26), Color.White, r, new Vector2(28 * .5f, 26 * .5f), scale, 0, 0);
			
			// Draws the laser 'head'
			spriteBatch.Draw(texture, start + (Distance + step) * unit - Main.screenPosition - F,
		...
		private void SpawnDusts(Player player)
		{
			...
			for (int i = 0; i < 5; ++i) {
				...
				Dust dust = Main.dust[Terraria.Dust.NewDust(dustPos, 0, 0, 264, dustVel.X, dustVel.Y, 0, new Color(255, 200, 0), 1f)];
				dust.noGravity = true;
				dust.scale = 2.2f;
			...
			if (Main.rand.NextBool(5)) {
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, 31, 0.0f, 0.0f, 100, Color.Red, 1.5f)];
				dust.velocity *= 0.5f;
				...
				dust = Main.dust[Dust.NewDust(Main.player[Projectile.owner].Center + 55 * unit, 8, 8, 31, 0.0f, 0.0f, 100, Color.Red, 1.5f)];
		...
		private void ChargeLaser(Player player)
		{
			...
			if (player.controlUseTile) {
				...
				*/
				...
				for (int k = 0; k < chargeFact + 5; k++)
				{
					Vector2 spawn = spawnPos + ((float)Main.rand.NextDouble() * 6.28f).ToRotationVector2() * (12f - chargeFact * 2);
					//Dust dust = Main.dust[Dust.NewDust(pos, 20, 20, 226, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f)];
					Dust dust = Main.dust[Terraria.Dust.NewDust(FixPos, 15, 15, 264, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f, 0, new Color(255, 200, 0), 1f)];
					dust.velocity = Vector2.Normalize(spawnPos - spawn) * 1.5f * (10f - chargeFact * 2f) / 10f;
					dust.noGravity = true;
					//dust.scale = Main.rand.Next(10, 20) * 0.05f;
					dust.scale = 2.2f;
