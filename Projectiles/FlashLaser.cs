...
{
	public class FlashLaser : ModProjectile
	{
		private bool SpeedUp = false;

		private bool takeControl = false;
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("FlashLaser");     //The English name of the Projectile
		...
		public override void AI()
		{
			...
			//Dust dust = Main.dust[Terraria.Dust.NewDust(Projectile.Center, 0, 0, 264, Projectile.velocity.X, Projectile.velocity.Y, 0, new Color(255, 200, 0), 1f)];

			int dustType2 = 6;//Main.rand.Next(139, 143);
			int dustIndex2 = Dust.NewDust(new Vector2(Projectile.Center.X - 5, Projectile.Center.Y - 5), 0, 0, dustType2);
            ...
            if (player.controlDown && takeControl)
            {
				Projectile.velocity = new Vector2(0, 3);
            }

            if (player.controlUp && takeControl)
            {
				Projectile.velocity = new Vector2(0, -3);
			}

            if (player.controlLeft && takeControl)
            {
				Projectile.velocity = new Vector2(-3, 0);
			}

            if (player.controlRight && takeControl)
            {
				Projectile.velocity = new Vector2(3, 0);
			}
