...
namespace MegamanXWeaponry.Items.Weapon
{
	public class ZSaberBlack : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			base.item.value = Item.sellPrice(80, 0, 0);
			base.item.rare = 11;
			base.item.scale = 1f;
		...
		{
			base.DisplayName.SetDefault("Z-SaberBlack");
			base.Tooltip.SetDefault("It is a powerful saber with a dark energy blade\nHold to charge a powerfull special attack [c/3639bb: - Costs 25MP ]");
		}

		public override void AddRecipes()
		{
			ModRecipe recipe = new ModRecipe(mod);
			recipe.AddIngredient(mod.GetItem("ZSaber"));
		...
		public override void UseStyle(Player player)
		{
			if (player.itemAnimation == player.itemAnimationMax - 1 && player.controlUseItem)
			{
				...
				base.item.useAnimation = (int)(15f / (1f + charge / 40f));
			...
			if (charge >= 120 && !player.controlUseItem)
            {
				...
				if(player.statMana >= 25)
                {
					...
					item.shoot = base.mod.ProjectileType("ZSaberBlackProjectile");
				}
		...
		{
			damage = (int)((float)damage * (1f + charge / 40f));
		}

		public override void GetWeaponCrit(Player player, ref int crit)
		{
			crit = (int)((float)crit * (1f + charge / 40f));
		}
